...
function CanvasMotif1Base({ onActiveChange }: CanvasMotifProps) {
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="pmcTileLogin1" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#5CC4CC" />
          <stop offset="55%" stopColor="#3FA3AC" />
          <stop offset="100%" stopColor="#26808A" />
        </linearGradient>
        <radialGradient id="pmcCoreLogin1" cx="38%" cy="30%" r="78%">
          <stop offset="0%" stopColor="#69CDD4" />
          <stop offset="60%" stopColor="#3FA3AC" />
          <stop offset="100%" stopColor="#1F6E77" />
        </radialGradient>
        <linearGradient id="pmcGlossLogin1" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.34" />
          <stop offset="100%" stopColor="#FFFFFF" stopOpacity="0" />
        </linearGradient>
        <filter id="pmcLiftLogin1" x="-40%" y="-40%" width="180%" height="180%">
      ...
      {PMC_MODULES.map((module, index) => {
        if (module.ringIndex === null) {
          ...
          return (
            <g key={module.n} {...interactiveProps(module, index)}>
              <g filter="url(#pmcLiftLogin1)">
                <circle cx={CENTER} cy={CENTER} r={CORE_RADIUS} fill="url(#pmcCoreLogin1)" />
                <circle
                  className="pmc-edge"
                  cx={CENTER}
                  cy={CENTER}
        ...
        return (
          <g key={module.n} {...interactiveProps(module, index)}>
            <g filter="url(#pmcLiftLogin1)">
              <rect x={x} y={y} width={TILE_SIZE} height={TILE_SIZE} rx={16} fill="url(#pmcTileLogin1)" />
              <rect
                className="pmc-edge"
                x={x + 1}
                y={y + 1}
...
export default memo(CanvasMotif1Base);
